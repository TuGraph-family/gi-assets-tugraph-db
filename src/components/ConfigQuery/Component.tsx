...
const ConfigQuery: React.FC<QuickQueryProps> = ({
  languageServiceId,
  schemaServiceId,
}) => {
  const [form] = Form.useForm();

  const { updateContext, services, graph } = useContext();
  ...
  const handleValueChange = async (changeValues, allValues) => {
    ...
    if (schemaArr.length > 0) {
      ...
      if (property) {
        const tmpProperty = schemaArr[0].properties.find(
          (d) => d.name === property,
        );
        if (tmpProperty) {
          setState((draft) => {
          ...
          >
            ...
            >
              {schemaTypeData.map((schema) => {
                return (
                  <Option key={schema.labelName} value={schema.labelName}>
                    <img
                      src={typeImg['person']}
                      alt=""
                      className="img"
